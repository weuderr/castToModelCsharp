...
{
    [HandleError]
    public class CvtpSeguimentoController : ControleBase
    {
        #region Propriedades
            String NomeExecutavel = String.Empty;
            CvtpSeguimentoBll acessaDados = null;
            UtilsController utils = null;
        #endregion
        
        #region Construtor
            public CvtpSeguimentoController()
            {
                this.NomeExecutavel = "PNMBI009";
                this.acessaDados = new CvtpSeguimentoBll();
        ...
        #region Verbos HTTP
            ...
            /// <summary>
            /// Método responsável por retornar os dados de CvtpSeguimento
            /// </summary>
            /// <param name="cod_seg_cvtp"></param>
            /// <param name="relations"></param>
            /// <returns>JsonResult</returns>
            [HttpGet]
            public JsonResult GetData(string cod_seg_cvtp = "", string relations = "")
            {
                if (!string.IsNullOrEmpty(cod_seg_cvtp))
                {
                    try
                    {
                        var value = Convert.ToInt32(cod_seg_cvtp);
                        var lista = this.acessaDados.GetCvtpSeguimentoById(value, relations);
            ...
            /// <summary>
            /// Método responsável por retornar uma lista de CvtpSeguimento
            /// </summary>
                ...
                using (StreamReader reader = new StreamReader(Request.InputStream))
                {
                    ...
                    if (!string.IsNullOrEmpty(body))
                    {
                        try
                        {
                            ...
                            string relations = "";
                            var lista = this.acessaDados.GetCvtpSeguimentoWithObject(queryObj, relations);
            ...
            /// <summary>
            /// Método responsável por salvar os dados de CvtpSeguimento
            /// </summary>
            /// <returns>JsonResult</returns>
            [HttpPost]
            public JsonResult CreateData()
            {
                using (StreamReader reader = new StreamReader(Request.InputStream))
                {
                    ...
                    if (!string.IsNullOrEmpty(body))
                    {
                        try
                        {
                            var model = JsonConvert.DeserializeObject<CvtpSeguimentoViewModel>(body);
                            ...
                            var lista = this.acessaDados.AddCvtpSeguimento(model);
                            if(lista > 0)
            ...
            /// <summary>
            /// Método responsável por atualizar os dados de CvtpSeguimento
            /// </summary>
            /// <returns>JsonResult</returns>
            [HttpPut]
                ...
                using (StreamReader reader = new StreamReader(Request.InputStream))
                {
                    ...
                    if (!string.IsNullOrEmpty(body))
                    {
                        try
                        {
                            ...
                            var lista = this.acessaDados.UpdateCvtpSeguimento(model);
                            if(lista > 0)
                                return Json(new { success = true }, JsonRequestBehavior.AllowGet);
                        }
                        catch (Exception e)
            ...
            /// <summary>
            /// Método responsável por deletar os dados de CvtpSeguimento
            /// </summary>
            /// <param name="cod_seg_cvtp"></param>
            /// <returns>JsonResult</returns>
            [HttpDelete]
            public JsonResult DeleteData(string cod_seg_cvtp = "")
            {
                if (!string.IsNullOrEmpty(cod_seg_cvtp))
                {
                    try
                    {
                        var value = Convert.ToInt32(cod_seg_cvtp);
                        var success = this.acessaDados.DeleteCvtpSeguimento(value);
